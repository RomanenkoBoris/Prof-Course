...
public class C_CopyFile {
    public static void main(String[] args) {
        int i;
        try (// блок инициализации переменных которые реализуют интерфейс AutoClosable
                FileInputStream fis = new FileInputStream("test.txt");
                FileOutputStream fos = new FileOutputStream("copy-of-test.txt");){
            // try-блок где происходит вся работа
            // -1 - сигнал что байты во входном потоке закончились
            // fis.read() - считывает и возвращает 1 байт
            // fis.read(byte[]) - считывает данные в массив и возвращает количество
            // считанных байтов
            while (  (i = fis.read()) != -1   ){
                fos.write(i);
            }
